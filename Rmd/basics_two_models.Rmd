...
fc_mean_rgdp_diffyoy <- pluck(fc_raw_1, "forecast", "rgdp", "mean")
fc_mean_rgdp_yoy <- any_fc_2_fc_yoy(fc_mean_rgdp_diffyoy, rgdp_transformation = "diff_yoy", rgdp_level_ts = rgdp_level)

fc_mean_rgdp_diffyoy
fc_mean_rgdp_yoy
...
Finally if you want to allow as many observations as possible in each cv rounds (e.g. 41 observations in the first round, 40 in the second and 32 in the tenth round) you have to set to training length to per_cv_maxs as below:
...
one_specification_pcm <-  one_specification %>%
    mutate(cv_obj = pmap(list(fit, variables, lag, t_threshold),
                         ~ cv_var_from_one_row(var_data = all_data_for_VAR_estimation,
                                               fit = ..1,
                                               variables = ..2, lags = ..3,
