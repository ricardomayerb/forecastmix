...
ensemble_all_h_row <- tibble(varibles = list(table(unlist(boo$variables))),
                             lags = list(table(unlist(boo$lags))),
                             short_name = "ensemble",
                             target_mean_fc_yoy = ensemble_fc_rmse_weighted,
                             rmse_h = paste0("rmse_", 1:length(ensemble_fc_rmse_weighted)),
                             rmse = ensemble_rmse)

tidy_fcs_tbl <- dplyr::select(boo, c(short_name, target_mean_fc_yoy))


uuu <- unnest(tidy_fcs_tbl, target_mean_fc_yoy, .drop = FALSE
       )
