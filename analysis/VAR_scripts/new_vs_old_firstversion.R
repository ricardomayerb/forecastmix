...
arg_var_res_old <- readRDS("./analysis/VAR_output/edd_exercises/2018_exercise_2/from_older_version_code/Chile_by_step_12345.rds") 
...
sort(names(arg_var_res_tmp))
sort(names(arg_var_res_old_tmp))


bon <- rbind(arg_var_res_tmp, arg_var_res_old_tmp)

arg_old_and_new <- stack_models(list(arg_var_res_tmp, arg_var_res_old_tmp))

foo <- single_plot_rmse_all_h(arg_old_and_new, is_wide = TRUE)
print(foo)

premoo <- arg_old_and_new %>% 
  arrange(short_name, model_function) %>% 
  distinct(short_name, .keep_all = TRUE) %>% 
  arrange(rmse_1, rmse_2, rmse_3)

moo <- single_plot_rmse_all_h(premoo, is_wide = TRUE)
print(moo)
