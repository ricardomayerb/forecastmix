...
country <- "Uruguay"
bra_search_results_name <- "vr_Uruguay_auto_3s4_2s5_norest" 
...
tic()
cv_less_endobra <- cv_var_from_model_tbl(
  h = fc_horizon,
  n_cv = n_cv,
  training_length = training_length,
  models_tbl = bra_models_tbl_smaller,
...
quasi_ave_5_e <- ave_fc_from_cv(cv_less_endobra, best_n_to_keep = 5)
quasi_ave_10_e <- ave_fc_from_cv(cv_less_endobra, best_n_to_keep = 10)
quasi_ave_20_e <- ave_fc_from_cv(cv_less_endobra, best_n_to_keep = 20)
quasi_ave_30_e <- ave_fc_from_cv(cv_less_endobra, best_n_to_keep = 30)

print(quasi_ave_5_e$ave_by_h_fc)
print(quasi_ave_10_e$ave_by_h_fc)
print(quasi_ave_20_e$ave_by_h_fc)
print(quasi_ave_30_e$ave_by_h_fc)


target_yoy_realized_and_fc_e <- ts(c(rgdp_yoy_ts, quasi_ave_20_e$ave_by_h_fc), 
                                 frequency = frequency(rgdp_yoy_ts), 
                                 start = start(rgdp_yoy_ts))

year_2018 <- window(target_yoy_realized_and_fc_e, start = c(2018,1), end = c(2018,4))

year_2019 <- window(target_yoy_realized_and_fc_e, start = c(2019,1), end = c(2019,4))
year_2018
year_2019
mean(year_2018)
mean(year_2019)

saveRDS(list(target_yoy_realized_and_fc_e, quasi_ave_10_e, quasi_ave_20_e, 
             quasi_ave_30_e, bra_models_tbl_smaller), 
        file = paste0(country, "_quasi_ave_e.rds"))
