...
cv_arimax <- function(y_ts,
                      ...
                      use_full_sample_arimax_order = FALSE,
                      full_sample_arimax_order = NULL,
                      use_full_sample_monthly_order = TRUE) {
  
  
  print("Starting cv_arimax")
  print(paste("In cv_arimax method =", method))
  # 
  # print(paste("in cv_arimax use_full_sample_monthly_order:", 
  #             use_full_sample_monthly_order))
  
  # print(paste("in cvarimax auto_do_approximation:", 
  #             auto_do_approximation))
  # print(paste("in cvarimax auto_do_stepwise:", 
  #             auto_do_stepwise))
  ...
  this_lag <- length(xreg_lags) - 1
  print(paste("in cv_arimax this_lag:", this_lag))
  ...
  for (x in 1:number_of_xregs) {
    ...
    # print(paste("x =", x))
    # print(paste("in cv_arimax, using variable number", x) )
    this_arima_name <- vec_of_names[x]
    # print("in cv_arimax, x variables is")
    print(this_arima_name)
    # print("this_arima_name")
    # print(this_arima_name)
    
    if (use_full_sample_arimax_order) {
      arimax_vlos <- full_sample_arimax_order
      this_var_vlos <- arimax_vlos[arimax_vlos$variable == this_arima_name, ]
      # print(this_var_vlos)
      this_var_lag_vlos <- this_var_vlos[this_var_vlos$lag == this_lag, ]
      # print(this_var_lag_vlos)
      
      y_order <- this_var_lag_vlos[["arima_order"]][[1]]
      y_seasonal <- this_var_lag_vlos[["arima_seasonal"]][[1]]
    ...
    # print("in cv_arimax, x_series_monthly")
    # print(x_series_monthly)
    # print("in cv_arimax, x_time_monthly")
    # print(x_time_monthly)
    # print("in cv_arimax, x_series")
    # print(x_series)
    # print("in cv_arimax, x_time")
    # print(x_time)
    # print("in cv_arimax, y_ts")
    # print(y_ts)
    # print("in cv_arimax, y_time")
    # print(y_time)
    # print(" ")
    # print(" ")
    # print(" ")
    ...
    for (i in seq_along(1:n_cv)) {
      ...
      # print(paste("In cv_arimax, i of cv is", i))
      ...
      end_ym_of_training <- as.yearmon(end_yq_of_training) + 2/12
      
      x_offset_in_months <- lead_m_monthly_variables[x]

      
      end_ym_of_training_with_x_offset <- end_ym_of_training + x_offset_in_months/12
      
      final_year_month_of_training_x <- c(year(end_ym_of_training_with_x_offset),
                                          month(end_ym_of_training_with_x_offset))
      
      # print("in cv arimax end_yq_of_training")
      # print(end_yq_of_training)
      # print("in cv arimax end_ym_of_training")
      # print(end_ym_of_training)
      # print("in cv arimax x offset in months")
      # print(x_offset_in_months)
      # print("in cv arimax end_ym_of_training_with_x_offset")
      # print(end_ym_of_training_with_x_offset)
      # print("final_year_month_of_training_x")
      # print(final_year_month_of_training_x)
      # 
      # print(" ")
      
      
      x_series_monthly <- window(x_series_monthly, 
                                 end = final_year_month_of_training_x)
      ...
      if (use_demetra) {
        ...
        this_x_extended_output <- get_extended_one_monthly_variable(
          ...
          use_full_sample_monthly_order = use_full_sample_monthly_order)
      ...
      if (!use_demetra) {
        ...
        if(use_full_sample_monthly_order) {
          this_x_extended_output <- get_extended_one_monthly_variable(
            monthly_variable_ts = x_series_monthly, 
            order_list = this_x_order_list, 
            use_demetra = FALSE,
          ...
          this_x_extended_output <- get_extended_one_monthly_variable(
            ...
            auto_do_approximation = auto_do_approximation, 
            auto_do_stepwise = auto_do_stepwise,
            use_full_sample_monthly_order = FALSE)
      ...
      # print("In cv_arimax, will start my_arimax")
      this_arimax_list <- my_arimax(
        y_ts = y_ts,
        xreg_ts = x_series_with_fc, 
        y_order = y_order, 
        ...
        auto_do_approximation = auto_do_approximation, 
        auto_do_stepwise = auto_do_stepwise, 
        use_previous_arimax_order = use_full_sample_arimax_order, 
        previous_arimax_order = full_sample_arimax_order)
      ...
      # print("in cv-arimax, storing this arimax")

      this_arimax <- this_arimax_list[[names(this_arimax_list)]]
      
      
      # print("in cv_arimax, will start forecast_xreg")
...
cv_arima <- function(y_ts,  h_max, n_cv, 
                     training_length,
                     y_order, y_seasonal,
                     ...
                     auto_do_approximation = TRUE,
                     auto_do_stepwise = TRUE,
                     for_uncond_use_previous_order = TRUE) {

  print("starting cv_arima")
  # print(paste("in cv_arima auto_do_approximation:", auto_do_approximation))
  # print(paste("in cv_arima for_uncond_use_previous_order", for_uncond_use_previous_order))
  ...
  for (i in seq_along(1:n_cv)) {
    ...
    if (force.constant & (this_d + this_D >= 2)) {
      ...
      if(for_uncond_use_previous_order) {
        this_arima <- Arima(training_y, order = y_order,
                            seasonal = y_seasonal,
                            method = method,
                            include.drift =  y_include_drift)
      } else {
        this_arima <- auto.arima(training_y, stepwise = auto_do_stepwise,
                                 approximation = auto_do_approximation)
      }
...
cv_of_ensemble <- function(var_training_length, 
  ...
  cv_ticks_lists_arima <- make_test_dates_list(var_data, n = n_cv, h_max = h_arima,
                                               training_length = arima_training_length)
...
fit_arimas <- function(y_ts, auto = FALSE,
                       order_list = NULL, 
                       my_lambda = NULL,
                       my_biasadj = FALSE, 
  ...
  # print(paste("in fitarimas, method is", method))
  
  # print(paste("in fitarimas, do_approximations is", do_approximation))
  # print(paste("in fitarimas, do_stepwise is", do_stepwise))
  # 
  # 
  # print("in fit arimas, order list is")
  # print(order_list)
  # 
  # print("in fit arimas, auto is")
  # print(auto)
  
  # print(paste("In fit_arimas, method =", method))
    ...
    if (!auto) {
      ...
      if (force_constant & (this_D + this_d) >= 2) {
        ...
        if (class_fit_1 == "try-error") {
          ...
          new_method <- "CSS-ML"
          this_msg <- paste0(method, " failed for ",  this_arima_names[i], ". Switching to CSS-ML")
          warning(this_msg)
          fit <- try(Arima(y = this_y, order = this_order, seasonal = this_seasonal,
                           include.constant =  include.constant, lambda = my_lambda, 
          ...
          if (class_fit_2 == "try-error") {
            print(paste(new_method, "failed"))
            new_new_method <- "CSS"
            this_msg <- paste0(new_method, " failed for ",  this_arima_names[i], ". Switching to CSS")
            warning(this_msg)
            fit <- try(Arima(y = this_y, order = this_order, seasonal = this_seasonal,
                             include.constant =  include.constant, lambda = my_lambda, 
            ...
            if (class_fit_3 == "try-error") {
              print(paste(new_new_method, " failed, enter auto.arima"))
              this_msg <- paste0(new_new_method, " failed for ",  this_arima_names[i], ". Switching to auto.arima with default values")
              warning(this_msg)
      ...
      print(paste("In fit_arima fitting auto arima for variable:", this_arima_names[i]))
      # print(paste("This is variable number", i, "out of", n_of_series))
      # print(paste("do_stepwise:", do_stepwise))
      # print(paste("do_approximation:", do_approximation))
      # print("order_list")
      # print(order_list)
      
      # tic()
      if (is.null(order_list)) {
        fit <- auto.arima(y = this_y, lambda = my_lambda, 
                          biasadj = my_biasadj, 
        ...
        "Using a pre-defined order structure"
        this_order <- order_list$order
        this_seasonal <- order_list$seasonal
        # print("foo1")
        fit <- auto.arima(y = this_y, lambda = my_lambda, 
                          biasadj = my_biasadj, 
                          stepwise = do_stepwise, 
                          ...
                          d = this_order[2],
                          D = this_seasonal[2],
                          start.p = this_order[1],
                          start.q = this_order[3],
                          start.P = this_seasonal[1],
                          start.Q = this_seasonal[3]
        )
        # print("foo2")
      }
...
fit_VAR_Arima <- function(arima_rgdp_ts, model_function, variables, 
  ...
  if (model_function == "Arima") {
    # print("in fit var arima, if arima")
...
get_arima_results <- function(country_name, read_results = FALSE, 
                              ...
                              method_in_extend_full_sample_monthly = "ML",
                              use_rgdp_uncond_order = FALSE,
                              arimax_auto_do_stepwise = TRUE,
                              arimax_auto_do_approximation = TRUE,
                              method_in_full_sample_arimax = "ML",
                              cv_arimax_auto_do_stepwise = TRUE,
                              cv_arimax_auto_do_approximation = TRUE,
                              method_in_cv = "ML",
                              cv_use_full_sample_arimax_order = TRUE,
                              cv_use_full_sample_monthly_order = TRUE
) {
  ...
  if (read_results) {
    ...
    print("In get_arima_results, final forcast horizon is")
    ...
    if (use_demetra) {
      ...
      fit_arima_rgdp_list_auto <- fit_arimas(
        ...
        do_stepwise = rgdp_auto_do_stepwise)
    ...
    extended_data <- get_extended_monthly_variables(
      do_auto = do_auto,
      method = method_in_extend_full_sample_monthly,
      use_demetra = use_demetra,
      do_dm_strict = do_dm_strict,
      do_dm_force_constant = use_dm_force_constant,
      monthly_data_ts = this_internal_monthly_ts, 
    ...
    arimax_and_fcs <- get_arimax_and_fcs(
      ...
      max_x_lag = max_x_lag,
      rgdp_order_list = rgdp_order_list,
      method = method_in_full_sample_arimax,
      h_max = h_max,
      data_is_log_log = data_is_log_log,
      force.constant = use_dm_force_constant, 
      auto_do_approximation = arimax_auto_do_approximation, 
      auto_do_stepwise = arimax_auto_do_stepwise, 
      do_auto = do_auto,
      use_rgdp_uncond_order = use_rgdp_uncond_order)
    toc()
    
    vlos_with_rgdp <- arimax_and_fcs$var_lag_order_season
    vlos <- vlos_with_rgdp[2:nrow(vlos_with_rgdp), ]
    ...
    cv_cond_uncond <- get_cv_obj_cond_uncond(
      ...
      auto_do_approximation = cv_arimax_auto_do_approximation,
      auto_do_stepwise = cv_arimax_auto_do_stepwise,
      use_full_sample_arimax_order = cv_use_full_sample_arimax_order,
      full_sample_arimax_order = vlos, 
      use_full_sample_monthly_order = cv_use_full_sample_monthly_order)
    toc()
...
get_arima_results_penultimo <- function(country_name, read_results = FALSE, 
                              data_folder = "./data/excel/",
                              arima_rds_path = "data/sarimax_objects_",
                              final_ext_horizon = c(2019, 12),
                              train_span = 16, number_of_cv = 8,
...
get_arimax_and_fcs <- function(y_ts, xreg_ts, 
                               rgdp_arima, 
                               max_x_lag,
                               rgdp_order_list, h_max, 
                               force.constant = FALSE,
                               data_is_log_log = FALSE,
                               auto_do_stepwise = FALSE,
                               auto_do_approximation = FALSE,
                               use_rgdp_uncond_order = TRUE,
                               do_auto = FALSE,
                               method = "ML"
                               ) {
  
  print("Starting get_arimax_and_fcs")
  # print("in get arimax and fcs")
  # print("do auto")
  # print(do_auto)
  ...
  for (i in 0:max_x_lag) {
    ...
    if (do_auto) {
      print("auto")
      if (use_rgdp_uncond_order) {
        print("using order from uncond rgdp")
        this_arimax <- my_arimax(
          y_ts = y_ts, xreg_ts = xreg_ts, y_order = rgdp_order, 
          y_seasonal = rgdp_seasonal, vec_of_names = monthly_names,
          force.constant = FALSE, y_include_mean = TRUE,
          xreg_lags = 0:i, do_auto = FALSE, method = method)
      }
      
      if (!use_rgdp_uncond_order) {
        # print("NOT using order from uncond rgdp. Estimate everything with xs")
...
get_cv_of_arimax <- function(y_ts, xreg_ts, 
                             xreg_ts_monthly, 
                             y_order,
                             y_seasonal,
                             x_names, 
  ...
  # 
  # print(paste("in gcoa auto_do_approximation:", 
  #             auto_do_approximation))
  # 
  # print(paste("in gcoa auto_do_stepwise:", 
  #             auto_do_stepwise))
  # 
  # print(paste("in gcoa use_full_sample_monthly_order:", 
  #             use_full_sample_monthly_order))
  ...
  for (i in 0:max_x_lag) {
    print(paste("in get_cv_of_arimax, current max_lag is",i))

    this_cv_arimax <- cv_arimax(
      y_ts = y_ts, 
      xreg_ts = xreg_ts,
      ...
      auto_do_approximation = auto_do_approximation,
      auto_do_stepwise = auto_do_stepwise,
      use_full_sample_arimax_order = use_full_sample_arimax_order,
      full_sample_arimax_order = full_sample_arimax_order,
      use_full_sample_monthly_order = use_full_sample_monthly_order)
...
get_cv_obj_cond_uncond <- function(y_ts, 
                                   xreg_ts, 
                                   xreg_ts_monthly,
                                   rgdp_arima,
                                   max_x_lag, 
                                   ...
                                   full_sample_arimax_order = NULL,
                                   for_uncond_use_previous_order = TRUE,
                                   uncond_previous_order = NULL,
                                   use_full_sample_monthly_order = TRUE) {
  
  print("Starting get_cv_obj_cond_uncond")
  
  # print(paste("in gcocu auto_do_approximation:", 
  #             auto_do_approximation))
  # 
  # print(paste("in gcocu auto_do_stepwise:", 
  #             auto_do_stepwise))
  # 
  # print(paste("in gcocu use_full_sample_monthly_order:", 
  #             use_full_sample_monthly_order))
  ...
  rgdp_order <- rgdp_order_list[["order"]]
  rgdp_seasonal <- rgdp_order_list[["seasonal"]]
  rgdp_mean_logical <- rgdp_order_list[["mean_logical"]]
  monthly_names <- colnames(xreg_ts)
  
  if (is.null(lead_m_monthly_variables)) {
    lead_m_monthly_variables <- rep(0, ncol(xreg_ts))
  }
  ...
  cv_rgdp_arima_list <- cv_arima(
    ...
    auto_do_approximation = auto_do_approximation,
    auto_do_stepwise = auto_do_stepwise,
    for_uncond_use_previous_order = for_uncond_use_previous_order)
...
get_extended_one_monthly_variable <- function(
  ...
  # print(paste("in geomv use_full_sample_monthly_order:", 
  #             use_full_sample_monthly_order))
  ...
  if (do_auto) {
    ...
    # tic()
    if (use_full_sample_monthly_order) {
      # print("in get_extended_one_monthly")
      # print("using previously determined x monthy order list for fit arimas")
      ...
      print("in get_extended_one_monthly")
      print("using auto.arima for fit arimas")
...
get_extended_monthly_variables <- function(
  ...
  auto_do_stepwise = FALSE, auto_do_approximation = FALSE) {
  
  # print("Starting get_extended_monthly_variable")
  # print(paste("in gemv auto_do_approximation:", 
  #             auto_do_approximation))
  # print(paste("in gemv auto_do_stepwise:", 
  #             auto_do_stepwise))
  ...
  if (do_auto) {
    # print("in get extended monthly variables")
    # print("doing auto")
    # print("auto_do_stepwise")
    # print(auto_do_stepwise)
    # print("auto_do_approximation")
    ...
    # print("in get extended monthly variables, will start fitting domestic monthly variables")
    # 
    # print("monthly_data_names")
    # print(monthly_data_names)

    fit_arima_monthly_list_auto <- fit_arimas(
      y_ts = monthly_data_ts, auto = TRUE, my_lambda = NULL, 
      do_approximation = auto_do_approximation, 
      do_stepwise = auto_do_stepwise,
      freq = 12, this_arima_names = monthly_data_names)
    
    # print("back to get ext mon var")
    # 
    # print("in get extended monthly variables, will start fitting external monthly variables")
    # 
    # print("monthly_data_external_names")
    # print(monthly_data_external_names)
    ...
    fit_arima_external_monthly_list_auto <- fit_arimas(
      ...
      do_approximation = auto_do_approximation, 
      do_stepwise = auto_do_stepwise,
      freq = 12, this_arima_names = monthly_data_external_names)
...
get_rgdp_and_dates <- function(data_path, gdp_name = "rgdp", 
  ...
  start_year_gdp <- first(data$year)
  start_quarter_gdp <- first(data$quarter)
  
  end_year_gdp <- last(data$year)
  end_quarter_gdp <- last(data$quarter)
...
list_of_rmse_plots <- function(selected_models_tbl, extra_models = NULL) {
  
  if (!is.null(extra_models)) {
    extra_models <- extra_models %>% 
      select(variables, lags, model_function, rmse_h, rmse, horizon) %>% 
      mutate(short_name = variables)
    
    selected_models_tbl <- selected_models_tbl %>% 
      select(variables, lags, model_function, rmse_h, rmse, horizon, short_name) 
    
    selected_models_tbl <- rbind(selected_models_tbl, extra_models)
  }
...
make_models_tbl <- function(arima_res, var_models_and_rmse, VAR_data, h_max, 
  ...
  # print("c(nrow(var_models_and_rmse))")
  # print(c(nrow(var_models_and_rmse)))
...
my_arima_one_x <- function(y_ts, y_order, y_seasonal, xreg_lags, x_name,
  ...
  # print("about to estimate, rgdp")

  if (x_name == "rgdp") {
    this_arimax <- try(Arima(y = y_ts, order = y_order, 
                         seasonal = y_seasonal, method = method))
    class_rgdp <- class(this_arimax)[1]
    if (class_rgdp == "try-error") {
      this_mssg <- paste0("For rgdp ML method failed in Arima. Switched to CSS-ML.")
      print(this_mssg)
      warning(this_mssg)
 
      new_method <-  "CSS-ML"
      this_arimax <- try(Arima(y = procrustean_y, order = y_order,
                               seasonal = y_seasonal,
                               method = new_method))
      
      class_rgdp <- class(this_arimax)[1]
      if (class_rgdp == "try-error") {
        this_mssg <- paste0("For rgdp CSS-ML method failed in Arima. Switched to CSS.")
        print(this_mssg)
        warning(this_mssg)
        new_new_method <-  "CSS-ML"
        this_arimax <- try(Arima(y = procrustean_y, order = y_order,
                                 seasonal = y_seasonal,
        ...
        class_rgdp <- class(this_arimax)[1]
        if (class_rgdp == "try-error") {
          this_mssg <- paste0("For rgdp CSS method failed in Arima. Switched to auto.arima with default values.")
          print(this_mssg)
          warning(this_mssg)
  ...
  # print("in my arima one x, about to start estimating arimax")
...
my_arimax <- function(y_ts, 
                      ...
                      auto_do_approximation = FALSE, 
                      auto_do_stepwise = FALSE,
                      use_previous_arimax_order = FALSE,
                      previous_arimax_order = NULL) {
  
  # print("Starting my_arimax")
  # print(paste("In my_arimax, method =", method))
  
  
  i = 1
  
  # print(paste("in myarimax, auto_do_approximation", auto_do_approximation))
  # print(paste("in myarimax, auto_do_stepwise", auto_do_stepwise))
  ...
  for (x_regressor in 1:number_of_xregs) {
    ...
    if (is.null(procrust_start)) {
      ...
      procrustean_y <- window(y_ts, start = latest_start, 
                              end = earliest_end, frequency = 4)
    ...
    if (do_auto) {
      ...
      if (use_previous_arimax_order) {
        # print(paste("fitting arimax with Arima + order from full sample arima. Xvar is", 
        #             vec_of_names[x_regressor], "(", x_regressor ,
        #             "of", number_of_xregs, ")",
        #             ". Lags go from 0 to", max(xreg_lags)))
        
        sel_var <- previous_arimax_order$variable == vec_of_names[x_regressor]
        sel_lag <- previous_arimax_order$lag ==  max(xreg_lags)
        sel_var_lag <- sel_var & sel_lag
        
        this_arimax_order <- previous_arimax_order[sel_var_lag, ]
        # print("this_arimax_order")
        # print(this_arimax_order)
        
        
        y_order <- this_arimax_order[["arima_order"]][[1]]
        y_seasonal <- this_arimax_order[["arima_seasonal"]][[1]] 
        ...
        this_arimax <- try(Arima(y = procrustean_y, 
                                 xreg = procrustean_x_and_lags,
                                 order = y_order,
                                 seasonal = y_seasonal,
        ...
        # print(paste("fitting arimax with auto.arima. xvar is", 
        #             vec_of_names[x_regressor], "(", x_regressor ,
        #             "of", number_of_xregs, ")",
        #             ". Lags go from 0 to", max(xreg_lags)))
        
        this_arimax <- auto.arima(
          y = procrustean_y, 
          xreg = procrustean_x_and_lags,
          stepwise = auto_do_stepwise, 
          approximation = auto_do_approximation
        )
    ...
    if (!do_auto) {
      this_arimax <- try(Arima(y = procrustean_y, 
                               ...
                               order = y_order,
                               seasonal = y_seasonal,
                               include.constant = y_include_mean,
                               method = method))
    ...
    if (class_this_arimax == "try-error") {
      print("Well, we have an error when fitting arimax in my_arimax")
      this_mssg <- paste0("For xreg variable ", vec_of_names[x_regressor], 
                          ", ", method, " method failed in Arima. Switched to CSS-ML.")
      warning(this_mssg)
      ...
      if (class_this_arimax == "try-error") {
        ...
        this_mssg <- paste0("For xreg variable ", vec_of_names[x_regressor], 
                            ", ", new_method, " method failed in Arima. Switched to CSS.")
        warning(this_mssg)
...
ts_data_for_arima <- function(data_path, external_data_path, all_logs = FALSE,
  ...
  n_series <- ncol(monthly_ts)
  n_series_external <- ncol(external_monthly_ts)
  
  max_time_rgdp <- max(time(rgdp_ts))
  max_time_rgdp_as_yq <- as.yearqtr(max_time_rgdp) 
  max_time_rgdp_as_ym <- as.yearmon(max_time_rgdp_as_yq) + 2/12
  

  lead_q <- rep(0, n_series)
  lead_m <- rep(0, n_series)
  lead_q_external <- rep(0, n_series_external)
  lead_m_external <- rep(0, n_series_external)
  
  for (i in 1:n_series) {
    this_series <- na.omit(monthly_ts[,i])
    this_max <- max(time(this_series))
    this_max_ym <- as.yearmon(this_max)
    this_max_yq <- as.yearqtr(this_max_ym)
    diff_q <- 4*(this_max_yq - max_time_rgdp_as_yq)
    lead_q[i] <- diff_q
    diff_m <- 12*(this_max_ym - max_time_rgdp_as_ym)
    lead_m[i] <- diff_m
    
  }
  
  
  for (i in 1:n_series_external) {
    this_series_external <- na.omit(external_monthly_ts[,i])
    this_max_external <- max(time(this_series_external))
    this_max_ym_external <- as.yearmon(this_max_external)
    this_max_yq_external <- as.yearqtr(this_max_ym_external)
    diff_q_external <- 4*(this_max_yq_external - max_time_rgdp_as_yq)
    lead_q_external[i] <- diff_q_external
    diff_m_external <- 12*(this_max_ym_external - max_time_rgdp_as_ym)
    lead_m_external[i] <- diff_m_external
  }
  
  names(lead_q) <- colnames(monthly_ts)
  ...
  return(list(
    ...
    rgdp_ts = rgdp_ts,
    lead_q = lead_q,
    lead_q_external = lead_q_external,
    lead_m = lead_m,
    lead_m_external = lead_m_external))
}
