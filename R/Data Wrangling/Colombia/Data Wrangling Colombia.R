...
# ise causes NA's in the VAR estimation so i drop it here
country_data_ts <- get_raw_data_ts(id = "Colombia", 
...
saveRDS(data_ts, "./data/raw_VAR_data/raw_VAR_data_Colombia.rds")
...
trend_recom <- c("rgdp", "rpc", "rgc", "serv", "uso_cap", "exp_notradictional", "imp_consumer", "imp_consumer_nd",
                 "imp_consumer_d")
...
saveRDS(final_VAR_data_for_estimation, "./data/VAR_data_Colombia.rds")

# We need to save the transformation done to rgdp. In the case of colombia we used the trend recom
target_transformation  <- rgdp_rec_trend
country <- id

saveRDS(object = list(country_name = country, 
                      target_transformation = target_transformation), 
        file = "./data/target_transformation/target_transformation_Colombia.rds")
