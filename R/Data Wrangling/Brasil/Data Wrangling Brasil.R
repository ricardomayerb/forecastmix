...
# emae or emae_sa, which one should delete. for now we keep both
country_data_ts <- get_raw_data_ts(id = "Brasil", 
                                   data_path = "./data/excel/Brasil.xlsx",
                                   general_variables_to_drop = c("year", "quarter", "hlookup", "rgdp_sa", "trim", 
                                                                 "month", "exist", "conf_emp", "conf_ibre"),
...
saveRDS(data_ts, "./data/raw_VAR_data/raw_VAR_data_Brasil.rds")
...
# I end up with datasets of different lenghts? How do we make sure that we do not mess up dates?
...
trend_recom <- c("rgdp", "rpc", "fbcf", "rm", "manuf", "tcr", "exp", "imp_consumer")
...
# common_span_ts <- ts.union(VAR_data_for_estimation_level[,1], VAR_data_for_estimation_trend[,1])
# VAR_data_for_estimation_level <- ts.union(common_span_ts, VAR_data_for_estimation_level)
# VAR_data_for_estimation_trend <- ts.union(common_span_ts, VAR_data_for_estimation_trend)
# VAR_data_for_estimation_level <- VAR_data_for_estimation_level[, -c(1,2)]
# VAR_data_for_estimation_trend <- VAR_data_for_estimation_trend[, -c(1,2)]
# 
# colnames(VAR_data_for_estimation_level) <- colnames(country_transformed_data_level)
# colnames(VAR_data_for_estimation_trend) <- colnames(country_transformed_data_trend)
...
saveRDS(final_VAR_data_for_estimation, "./data/VAR_data_Brasil.rds")

# We need to save the transformation done to rgdp. In this case we switched to the trend recom
target_transformation  <- reco_all_variables_trend[reco_all_variables_trend$variable == "rgdp", ][["this_test_alpha_deter"]]
country <- id

saveRDS(object = list(country_name = country, 
                      target_transformation = target_transformation), 
        file = "./data/target_transformation/target_transformation_Brasil.rds")